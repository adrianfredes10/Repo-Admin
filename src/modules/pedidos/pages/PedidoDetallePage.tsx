...
export const PedidoDetallePage = () => {
  ...
  // Con MercadoPago el admin NO confirma a mano: lo hace el pago aprobado (el
  // backend rechaza el "Confirmar" manual). En ese caso dejamos solo Cancelar.
  const esMpPendiente =
    pedido.estado.codigo === 'PENDIENTE' && pedido.forma_pago.codigo === 'MERCADOPAGO';
  const acciones = (ACCIONES[pedido.estado.codigo] ?? []).filter(
    (accion) => !esMpPendiente || accion.cancel,
  );
  const errorAccion = avanzar.error ?? cancelar.error;
          ...
          <div className="mt-6 pt-5 border-t border-gray-100">
            ...
            {esMpPendiente && (
              <p className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 mb-3">
                💳 Pago con MercadoPago: el pedido se confirma automáticamente cuando el pago es aprobado, no manualmente.
              </p>
            )}
            {errorAccion && (
              <p className="text-xs text-red-600 mb-3">
                {errorAccion instanceof Error ? errorAccion.message : 'No se pudo cambiar el estado'}
              </p>
            )}
