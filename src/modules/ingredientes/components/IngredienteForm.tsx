...
export const IngredienteForm = ({ onSubmit, initialData, isLoading = false }: Props) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!['ml', 'g', 'unidades'].includes(unidad_medida)) { setError('Unidad de medida no válida'); return; }
    if (stock_actual < 0) { setError('El stock actual no puede ser negativo'); return; }
    if (stock_minimo < 0) { setError('El stock mínimo no puede ser negativo'); return; }

    onSubmit({ nombre: nombre.trim(), unidad_medida, stock_actual, stock_minimo });
  };
